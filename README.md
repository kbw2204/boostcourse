...
struct과 같은 값 타입은 값을 전달할 때마다 값을 복사해 전달하는것과 달리 class와 같은 참조 타입은 하나의 인스턴스가 참조를 통해 참조가 되기 떄문에 언제 메모리에서 해제되는지가 중요합니다. Swift는 프로그램의 메모리 사용을 관리하기 위해 메모리 관리 기법인 ARC를 사용합니다.
